...
from common_utils import press_enter  # 导入press_enter函数
...
def format_subscription_type(subscription_data: Dict) -> str:
    ...
    if "subscription" in subscription_data:
        ...
        if status == "active":
            if subscription.get("plan", {}).get("nickname", "").lower() == "pro":
                return "Pro"
            elif subscription.get("plan", {}).get("nickname", "").lower() == "pro trial":
                return "Pro Trial"
            else:
                return "Active"
        else:
