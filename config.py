...
def setup_config(translator=None):
    ...
    try:
        ...
        # normalize path
        config_dir = os.path.normpath(os.path.join(docs_path, ".cursor-manager"))
        ...
        try:
            ...
            # Suppressing the config directory creation message
            ...
            import tempfile
            temp_dir = os.path.normpath(os.path.join(tempfile.gettempdir(), ".cursor-manager"))
        ...
        if os.path.exists(config_file):
            ...
            with open(config_file, 'w', encoding='utf-8') as f:
                config.write(f)
            # Suppressing the config file creation message
            # if translator:
...
def print_config(config, translator=None):
    ...
    print(f"\n{Fore.CYAN}{'─' * 70}{Style.RESET_ALL}")
    config_dir = os.path.join(get_user_documents_path(), ".cursor-manager", "config.ini")
...
def force_update_config(translator=None):
    ...
    """
    try:
        config_dir = os.path.join(get_user_documents_path(), ".cursor-manager")
